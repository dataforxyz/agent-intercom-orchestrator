...
Tool-based create accepts an optional structured `requirements` object: `worktree` and `gitTransport` take `read` or `write`, while `edit` and `tests` are booleans. Boss never infers these requirements from free-form goal text. Requested requirements are checked after general readiness but before any run is persisted or participant is launched, and the report keeps each requested level separate from its effectively probed `available` or `unavailable` result. Linked-worktree identity comes from Git administrative metadata; effective read/write/edit access combines filesystem probes with the exact Worker permission/tool policy, so a read-only worktree visibly blocks write or edit work. Boss does not create worktrees. Project-specific tests and Git transport remain unavailable unless an exact future probe establishes them; configured shell/Git policy alone is not treated as proof. Any unavailable requested capability returns `BOSS_CAPABILITY_GAP` and creates no run. Successful requested checks are returned in `details.capabilityReport`; later status continues to report runtime/communication evidence and does not reinterpret that create-time preflight as completed work. The interactive `/boss create` grammar remains goal-only, so use the tool when structured requirements matter.

...
Built-in roles now select named permission profiles. `review-readonly` makes the host and assigned workspace read-only except for private temp and harness runtime state, limits Pi to inspection/Intercom tools, hides common credential paths, and blocks Git/GitHub mutations. `builder-restricted` makes only the assigned workspace and harness runtime state writable, mounts Git metadata read-only, and applies the same credential and Git/GitHub guards. `trusted` preserves broad host access. Custom roles default to `builder-restricted` unless `trusted` is selected explicitly. Hardened profiles also rebuild the worker environment from an allowlist, give each worker a private home and harness state, proxy Intercom through a short private broker socket, mask the user and system D-Bus control sockets, and isolate PIDs so workers cannot delegate an unsandboxed service to the host user manager. Nested namespaces remain available for harness sandboxes such as Codex, but inherit the outer read-only mounts and cannot recover the masked host control sockets. Hardened profiles require systemd 257+ and `/usr/bin/bwrap`; the supervisor uses a nested mount namespace to hide the source Intercom directory after opening the private broker proxy. The systemd filesystem restrictions and packaged `git`/`gh` PATH guards apply across harnesses; Pi also enforces the policy in its `tool_call` hook.

...
The LLM-callable create surface may additionally declare structured `requirements` for worktree access (`read` or `write`), edit access, tests, and Git transport (`read` or `write`). Nothing is inferred from the goal. Only explicit requirements are assessed, and every finding preserves the requested level separately from its effectively probed availability. The check runs before run persistence and staffing: linked-worktree identity is verified from Git administrative metadata, while filesystem probes are combined with the exact Worker permission/tool policy. A read-only exact worktree is therefore a blocking write/edit gap. Project-specific tests and Git transport remain unavailable when no exact probe establishes them; configured tools or policy are not promoted to proof. Any unavailable request returns `BOSS_CAPABILITY_GAP` with an itemized report and creates no trusted-local state. Successful reports are create-time evidence only and do not replace the runtime and communication fields exposed by status. The interactive slash grammar remains goal-only and cannot express this structured contract.

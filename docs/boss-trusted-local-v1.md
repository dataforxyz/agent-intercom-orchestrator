...
The LLM-callable create surface may additionally declare `needs: ["worktree", "edit", "test", "git-transport"]`. Only listed needs are assessed. The check runs before run persistence and staffing: linked-worktree metadata can be verified, while edit/test results are limited to the enforced Worker permission and tool configuration. Project-specific test tooling is not claimed as verified. Git transport is fail-closed because remote reachability, credentials, and write authority are not verified. Any gap returns `BOSS_CAPABILITY_GAP` with an itemized report and creates no trusted-local state. Successful reports are create-time evidence only and do not replace the runtime and communication fields exposed by status.

...
WorkerStore schema v3 is the first format allowed to persist `lastAuthenticatedIntercomActivityAt` as authenticated evidence. Current readers accept v1 and v2 as legacy inputs, construct a v3 view in memory, and make that migration durable on the explicit migration path or the next successful store mutation. Migration preserves lifecycle, identity, generation, ownership, cleanup claims, and ordinary lease activity, but deliberately discards any `lastAuthenticatedIntercomActivityAt` field found under a v1 or v2 header: neither legacy schema defined the authentication contract, so promoting that value would invent evidence.

...
WorkerStore schema v3 is the first format allowed to persist `lastAuthenticatedIntercomActivityAt` as authenticated evidence. The exact v1 and v2 schemas do not contain that key. Current readers accept valid v1 and v2 inputs, construct a v3 view in memory, and make that migration durable on the explicit migration path or the next successful store mutation. Migration preserves lifecycle, identity, generation, ownership, cleanup claims, and ordinary lease activity while leaving authenticated activity undefined; it never derives that evidence from `lastWorkerActivityAt` or another legacy field. For rollback safety, the reader also recognizes the briefly shipped v2 writer's timestamp-shaped field as compatibility-only input and discards it rather than promoting it to authenticated evidence or quarantining an otherwise valid store.

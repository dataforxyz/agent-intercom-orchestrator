...
### WorkerStore schema compatibility and rollback

WorkerStore schema v3 is the first format allowed to persist `lastAuthenticatedIntercomActivityAt` as authenticated evidence. The canonical v1 and v2 schemas do not contain that key. Current readers accept valid v1 and v2 inputs, construct a v3 view in memory, and make that migration durable on the explicit migration path or the next successful store mutation. Migration preserves lifecycle, identity, generation, ownership, cleanup claims, and ordinary lease activity while leaving authenticated activity undefined; it never derives that evidence from `lastWorkerActivityAt` or another legacy field. For rollback safety, the raw legacy readers also recognize the timestamp-shaped field accepted by the briefly shipped v1/v2 reader as compatibility-only input and discard it rather than promoting it to authenticated evidence or quarantining an otherwise valid store.

Schema and feature gates fail closed before recovery handling. A reader encountering v4 or later raises `WORKER_STORE_NEWER_SCHEMA` without rewriting, moving, quarantining, or poisoning the file. Likewise, an unsupported active feature prevents reads and mutations without quarantine. Install a reader that supports the observed schema or feature; do not edit the version header to bypass either gate.

Each populated v3 commit preserves the prior canonical state in `workers.json.recovery.json` before replacement. If the canonical registry is unexpectedly empty while managed units are still live, startup restores that predecessor only when every live unit has one exact owned identity match and no conflicting snapshot identity exists. The restore and a fresh cleanup grace window are one durable commit. Ambiguous, missing, or invalid evidence creates `worker-registry-diagnostic.json`, puts fleet operations into fail-closed read-only degraded mode, and lists the verified-but-untracked units and reason; inspect the canonical file, recovery file, diagnostic, and systemd units before repairing or quarantining evidence. Do not delete the diagnostic merely to bypass the fence.

Recovery has deliberate limits. Units created before the identity environment was introduced may not carry enough evidence and therefore degrade rather than auto-restore. An explicitly adopted worker may retain an older unit-level manager identity while its store owner has changed, which also requires operator reconciliation. The recovery file is a predecessor snapshot, not a journal, so it can lag the last canonical state by one commit. Finally, old-version processes writing concurrently can rotate or overwrite evidence using semantics this reader cannot make safe; stop all older managers and cleanup writers before upgrade, recovery, or rollback.

Treat v3 as a one-way operational upgrade for a live store. Before rolling the package back to a v2-only release, stop every manager, cleanup timer, and other WorkerStore writer. The preferred rollback is to upgrade the older binary so it can read v3. An installed v2 checkout intentionally refuses a v3 file; repointing an npm/git link or executable to that checkout is not a store rollback. Stage any installed-checkout downgrade against a copy first, record the package source reported by `agent_fleet({ action: "versions" })`, and preserve both the installed revision and a matching pre-upgrade v2 store backup. If package rollback is unavoidable, restore that coordinated v2 backup before starting the v2 checkout, accepting that every state change after the backup—including authenticated activity evidence—is lost. Never run v2-only and v3 writers against the same store, and never relabel a v3 file as v2.

Stopping preserves the worker record and supported harness session state for resume. It also removes disposable per-worker npm, pip, uv, and pnpm caches by default, avoiding retention of downloaded tool binaries while leaving transcripts and harness state intact. `stop` is always available and records best-effort dirty-worktree evidence for writable workers; it never refuses the safety operation because of Git state. The cleanup timer prunes clean terminal records after the configured retention period (7 days by default), dirty records after a longer period (30 days by default), and private runtime directories that remain without a worker record for 60 minutes by default. Runtime cleanup persists a same-ID reservation, verifies matching systemd units and cgroups are absent, rejects symlinked path ancestors, atomically renames selected paths into a private quarantine, and recovers interrupted move or deletion phases on the next startup. Candidate failures are isolated so one unsafe path does not block unrelated cleanup. `forget` is a distinct single-worker terminal action. `prune` bulk-removes terminal history in the current manager scope. Both explicit deletion actions require manager `acknowledge: true`.
...
Explicit models matching ordered `routing.modelRouting.rules` choose a direct harness; patterns are exact or use one safe trailing `*`. An optional `unmatchedHarness` handles every unmatched explicit model, while `null` preserves normal role routing. `stripPrefixes` controls direct-CLI normalization. OpenCode remains explicit-only regardless of configured automatic preferences; an explicit harness, profile, or matching explicit model may still select it. The preview explains every rank, warning, exclusion, selected profile/mode, and permission profile. Harness-specific role profile/model/effort settings do not cross a harness fallback. Role instructions cross only when `routing.fallback.preserveRoleInstructions` is enabled; explicit caller instructions always survive.

...
Leases are activity-bounded rather than manager-heartbeat-bounded. A manager-received worker Intercom message or explicit `renew` extends the lease, capped at the configured idle deadline; process existence, broker acknowledgements, and messages sent by the manager do not count. The manager requests a commit/checkpoint/handoff before the deadline, cleanup preserves a grace period for recovery or adoption, and a persistent systemd user timer applies global cleanup across every manager owner even when no manager session is running. Startup cleanup and `/agents-cleanup` use the same race-safe deadline check. Completed one-shot units are retired automatically after reconciliation so their retained exit status does not accumulate in systemd.

Global cleanup has four separately reported actions: `stop` for live owned workers past checkpoint grace, `cache` for disposable private-home caches of terminal workers, `full` for expired terminal private runtimes plus their exact records, and `orphan` for unregistered runtime directories. Preview is the default and reports each action, path set, and estimated bytes; use `execute: true` or `/agents-cleanup execute` to apply it. Defaults are 60 minutes for terminal caches, 7 days (`10080` minutes) for full terminal runtimes/records, and 60 minutes of runtime-directory age for unregistered runtimes. Configure these as `terminalCacheRetentionMinutes`, `terminalRuntimeRetentionMinutes`, and `orphanRuntimeRetentionMinutes`.

Stopping remains resumable: the worker record, transcript/session data, and other primary harness state survive cache pruning until full retention expires. Terminal cleanup first records a durable phased claim, then verifies that the exact unit and every same-ID unit are absent outside the cross-process state lock. It reacquires the lock only to revalidate the claim, persist the exact contained path indexes, and atomically rename those paths into a private quarantine; recursive deletion happens after the lock is released. Orphan cleanup uses the same persisted claim protocol while rechecking registration and the canonical runtime root. The unresolved claim fences every same-ID spawn until recursive removal and the final state commit complete, while unrelated managers and heartbeats remain unblocked. Crash recovery rolls a `moving` claim back to its canonical paths, resumes a `moved` or `deleting` claim, and never removes a quarantine based only on age. Partial renames and state-write failures therefore preserve the worker record and resumable runtime. Symlinked intermediate paths are rejected, and a runtime-root symlink is unlinked rather than followed. If systemd, cgroup, containment, or current-state absence cannot be proven, deletion is skipped safely for a later timer run; failure of one cleanup candidate is reported without aborting the remaining candidates.

Automatic startup and timer runs use directory/record metadata only when selecting retention work. They do not recursively walk retained runtime trees for sizing. An explicit cleanup preview performs the recursive size estimate and returns it in action details.

...
- When the caller did not explicitly choose routing constraints, send `harness: "auto"`, `effort: "auto"`, and `subagents: "auto"` (or omit them when the client preserves optional fields). Automatic routing checks configured ordered profiles, executable/runtime availability, effort, and capability requirements. Never invent `pi`, `off`, or `false` placeholders; explicit caller harness/profile choices win only when that harness is enabled. Explicit models use the configured ordered model rules and unmatched-model harness. Treat top-level `disabledHarnesses`, `explicitOnly`, profile order, role requirements, instruction fallback, and supervision recommendations as configuration—inspect them instead of assuming the defaults.
...
- `/agents-config` — enable or disable harnesses and edit per-harness defaults, lifecycle settings, and role presets
...
Top-level `disabledHarnesses` controls which harnesses may create new coworkers; disabled harnesses remain manageable for existing-worker lifecycle and cleanup operations. Routing configuration lives under `routing`: `preference` and `roles` order enabled harnesses, `explicitOnly` controls automatic exclusions (with OpenCode always explicit-only), `profilePreferences` orders launch-profile fallback, `roleRequirements` supplies capability defaults, `modelRouting` controls model inference and normalization (including an optional unmatched-model harness), `fallback` controls portable role instructions, and `capabilities` describes actual harness support. Existing defaults, role presets, and explicit spawn fields remain supported.

...
- Permission profiles are guardrails for ordinary coding-agent mistakes, not a hostile-code container. Restricted workers retain writable private temp and isolated per-worker harness state, a restricted builder can still damage files inside its assigned workspace, and direct absolute invocation of host binaries can bypass the PATH-level `git`/`gh`/`glab`/`tea` guards. Systemd's read-only host/Git mounts, hidden glab/Tea/Git hosting configuration, scrubbed inherited credentials, and explicit host container/VM daemon and host-mutating systemd/polkit socket masks are defense in depth—not a claim that hostile code cannot supply another credential or reach a separately network-exposed daemon API. Provider authentication intentionally copied into a private harness home is necessarily readable by that worker, while unrelated manager environment variables and host credential paths are excluded.

...
- Permission profiles are guardrails for ordinary coding-agent mistakes, not a hostile-code container. Restricted workers retain writable private temp and isolated per-worker harness state, a restricted builder can still damage files inside its assigned workspace, and direct absolute invocation of host binaries can bypass the PATH-level `git`/`gh` guards; systemd's read-only host/Git mounts and hidden credentials remain the backstop. Provider authentication copied into a private harness home is necessarily readable by that worker, while unrelated manager environment variables and host credential paths are excluded.
